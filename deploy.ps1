...
function UpdateHtmlReferences {
    ...
    try {
        ...
        $content = $content -replace '../../Core/', '../../site/Core/'
        
        # Ajouter le script de chargement différé des images
        $scriptPattern = '<script src="../../Core/navigation.js"></script>'
        $scriptReplacement = '<script src="../../site/Core/navigation.js"></script>' + "`r`n" + '    <script src="../../site/Core/image-loader.js"></script>'
...
Write-Host "Étape 5: Finalisation du déploiement..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Pour déployer le site sur un serveur web :" -ForegroundColor Cyan
Write-Host "1. Copiez uniquement le dossier 'site/' sur le serveur web" -ForegroundColor Cyan
Write-Host "2. Conservez le dossier 'ressources/' sur un stockage local ou un serveur de fichiers séparé" -ForegroundColor Cyan
Write-Host ""