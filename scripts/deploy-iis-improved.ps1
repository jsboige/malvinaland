...
function Fix-FilePaths {
    ...
    try {
        ...
        Write-ColorMessage "✓ Chemins corrigés dans $FilePath" -Color Green
    }
    catch {
...
function Fix-MobileMenu {
    ...
    try {
        ...
        Write-ColorMessage "✓ Menu mobile corrigé dans $FilePath" -Color Green
    }
    catch {
...
function Create-ReadmeFiles {
    ...
    foreach ($monde in $mondes) {
        ...
        if (-not (Test-Path -Path $mondeDir)) {
            ...
            Write-ColorMessage "✓ Répertoire créé : $mondeDir" -Color Green
        }
...
"@
            ...
            Write-ColorMessage "✓ Fichier README.md créé : $readmePath" -Color Green
        }
        ...
        if (-not (Test-Path -Path $imagesDir)) {
            ...
            Write-ColorMessage "✓ Répertoire images créé : $imagesDir" -Color Green
        }
    }
}
...
"@
    ...
    # Écrire le contenu dans le fichier
    Set-Content -Path $readmePath -Value $readmeContent -Encoding UTF8
    Write-ColorMessage "✓ README.md principal corrigé" -Color Green
}
...
function Deploy-ToIIS {
    ...
    try {
        ...
        Write-ColorMessage "✗ Erreur lors du déploiement sur IIS: $($_.Exception.Message)" -Color Red
        return $false
    }
}
...
function Reset-IIS {
    ...
    try {
        ...
        Write-ColorMessage "✓ Site arrêté" -Color Green
        ...
        Write-ColorMessage "✗ Erreur lors de la réinitialisation d'IIS: $($_.Exception.Message)" -Color Red
        return $false
    }
}
